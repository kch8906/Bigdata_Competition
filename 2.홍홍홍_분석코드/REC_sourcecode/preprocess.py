...
def preprocess():    
    df_02 = pd.read_csv('../../3.홍홍홍_데이터 및 모델 세이브 파일/dataset/LPOINT_BIG_COMP/LPOINT_BIG_COMP_02_PDDE.csv')    
    df_04 = pd.read_csv('../../3.홍홍홍_데이터 및 모델 세이브 파일/dataset/LPOINT_BIG_COMP_04_PD_CLAC.csv')    
    ...
    data_main = data.groupby("session").filter(lambda x: len(x) > 5)
    data_cold = data.groupby("session").filter(lambda x: len(x) <= 5)

    data_cold.to_csv('../../dataset/big_comp/cold_start.csv', sep='\t', index=False)
    data_main.to_csv('../../dataset/big_comp/main.csv', sep='\t', index=False)
